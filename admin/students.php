...
$params = [];
$types = '';
if ($q !== '') { $where[] = "(firstname LIKE ? OR lastname LIKE ? OR email LIKE ? OR phone LIKE ? OR payment_ref LIKE ? OR gcash_number LIKE ? OR guardian LIKE ? OR address LIKE ? OR school LIKE ?)"; $like = "%$q%"; for ($i=0;$i<9;$i++){ $params[] = $like; $types .= 's'; } }
if ($course !== '') { $where[] = 'course = ?'; $params[] = $course; $types .= 's'; }
if ($year !== '') { $where[] = 'year_level = ?'; $params[] = $year; $types .= 's'; }

$sql = 'SELECT id, token, firstname, lastname, middlename, address, sex, nationality, birthplace, birthdate, school, guardian, relationship, vaccination, email, phone, gcash_number, course, year_level, payment_ref, payment_proof, student_status, created_at FROM students';
...
<body>
    ...
    <main class="main">
      <div class="container">
        ...
        <form method="get" class="search-form">
          <input type="text" name="q" placeholder="Search name, email, phone, payment ref" value="<?php echo htmlspecialchars($q); ?>">
          <select name="course">
        ...
        <div class="table-panel">
          <table class="table">
            <thead><tr><th>ID</th><th>Name</th><th>Address</th><th>Sex</th><th>Birthdate</th><th>Guardian</th><th>Course / Year</th><th>GCash</th><th>Phone</th><th>Email</th><th>Payment Ref</th><th>Submitted</th><th>Actions</th></tr></thead>
            <tbody>
  ...
  <table class="table">
    <thead><tr><th>ID</th><th>Name</th><th>Course / Year</th><th>Email</th><th>Phone</th><th>Payment Ref</th><th>Submitted</th><th>Actions</th></tr></thead>
    <tbody>
    <?php while($r = $res->fetch_assoc()): ?>
      <tr>
        <td><?php echo $r['id']; ?></td>
